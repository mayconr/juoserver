...
public
class GameServerLoginHandler extends SimpleChannelInboundHandler<GameServerLogin> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, GameServerLogin msg) throws Exception {
        database.getAccount(msg.getUsername(), msg.getPassword()).ifPresentOrElse(account->{
            ...
            ctx.writeAndFlush(new CharacterList(players,
                    database.getCities(), CharacterListFlag.ENABLE_AOS_COMMON,
                    CharacterListFlag.SAMURAI_NINJA_CLASSES,
                    CharacterListFlag.ENABLE_NPC_POPUP,
                    CharacterListFlag.ELVEN_RACE));
        }, ()-> ctx.writeAndFlush(new LoginReject(LoginReject.Reason.COULD_NOT_ATTACH_SERVER)));
    }
}
