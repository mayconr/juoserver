...
public class UOItem extends UOObject {
    ...
    public UOItem(int serialId, int modelId, int x, int y, int z, String name, int amount, int hue) {
    ...
    public UOItem(int serialId, ItemPrototype prototype, Location location) {
        super(serialId, prototype.getModelId(), location.getX(), location.getY(), location.getZ(), prototype.getDisplayName());
        this.type = prototype.getType();
        this.movable = prototype.isMovable();
        this.hue = prototype.getHue();
        this.hidden = prototype.isHidden();
        this.mountNpc = Optional.ofNullable(prototype.getMount()).map(ItemPrototype.MountTypePrototype::getNpc).orElse(null);
    }
