...
public class Huffman implements Encoder {
     ...
     *
     * <p>The i-th element is an array:
     *
     * <ol>
     *   <li>whose second element is the encoding for i - if translated into binary notation, this
     *       number can be seen as the path within the tree to reach the "leaf" i from the root of
     *       the tree (e.g. where 0 is the left branch and 1 is the right branch)
     *   <li>and the first element is the number of significant bits within the path.
     * </ol>
     *
     ...
     *
     * <ul>
     *   <li>5 is encoded as 0x28 (= 101000 in binary notation),
     *   <li>which has (0x06 =) 6 significant bits.
     * </ul>
     *
     ...
     * </pre>
     *
     * Numbers with shorter paths (i.e. with a lower number of significant bits) are those who are
     * statistically less likely to appear (less probable), hence the algorithm ensures that we use
     * more space only a "few times", thus saving space (bytes).
     */
    static final short[][] encodeTree =
            new short[][] {
                {0x02, 0x00},
                {0x05, 0x1F},
                {0x06, 0x22},
     ...
     * @return a new array equal to the provided one, having the last byte appended at the end
     */
