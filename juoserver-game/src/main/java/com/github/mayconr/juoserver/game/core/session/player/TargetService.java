...
import com.github.mayconr.juoserver.game.packet.CursorTarget;
    ...
    public void handleTarget(Target target) {
        if (cursorId.get() == target.getCursorId()) {
            switch (target.getTarget()) {
                case LOCATION -> eventBus.publish(new SelectedStatics(mobile, target.getModelId(), target.getX(), target.getY(), target.getZ()));
                case OBJECT -> eventBus.publish(new SelectedObject(mobile, target.getClickedSerialId(), target.getX(), target.getY(), target.getZ()));
                default -> log.warn("Invalid cursor target [{}] for mobile [{}-{}]", target.getTarget(), mobile.getSerialId(), mobile.getName());
            }
        }
    }
}
